...
- Em `Resultados`, aplicamos á `FlatList` components personalizados para caso não houver filmes na busca (`NaoEncontrado/ListEmptyContent`) e para separar cada elemento da `FlatList` (`Separador/ItemSeparadorComponent`)
- Em `cardFilme` colocamos uma condicional para o carregamento da imagem alternativa caso algum filme não tenha imagem e também ícones nos botões.
